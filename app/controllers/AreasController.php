...
class AreasController extends \BaseController {
	...
	public function index()
	{
		return View::make('areas.index')
					->with('title','All Areas')
					->with('areas',Area::paginate(8));
	}
	...
	public function edit($id)
	{
		return View::make('areas.edit')
					->with('title','Edit An Area')
					->with('area',Area::where('id', '=', $id)->first());
	}
	...
	public function update($id)
	{
		...
		if($validation->fails())
			...
			$areaUpdate = ['area_name'=>Input::get('area')];
			
			if(Area::find(Input::get('area_id'))->update($areaUpdate))
				    return Redirect::route('area.index')
